...
- Confirmed fast-opencv ops: `frameBufferToMat`, `bufferToMat(type,rows,cols,ch,input)`, `base64ToMat`, `matToBuffer(mat,type)→{buffer}`, `invoke('cvtColor'|'warpAffine'|'resize', …)`. base64ToMat yields **BGR** (OpenCV-native) — matches MiniFASNet directly; cvtColor→RGB for recognition.
