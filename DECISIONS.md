...
- **2 — UI not light-themed.** `expo-glass-effect`'s `<GlassView>` renders the real iOS 26 Liquid Glass material only on iOS 26+; **everywhere else it degrades to a plain transparent `<View>`**, leaving the panels invisible over the camera (reads as "not light mode"). CLAUDE.md §3c already mandated providing the fallback look; it was missing. **Decision:** when `isLiquidGlassAvailable()` is false (Android / iOS <26), paint each glass surface as a solid light card (white-ish bg, hairline border, soft shadow; primary action → solid accent), plus a dark-icon `StatusBar`. iOS 26 keeps pure glass untouched.
